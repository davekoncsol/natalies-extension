        ...
        function getEventHandler(event, handler, context, selector) {
            ...
            var eventHandler = _events.filter(function (eventHandler) {
                for (var i = 0; i < getHandler.length; i++) {
                    if (eventHandler[i] === getHandler[i] || eqObjects(eventHandler[i], getHandler[i])) {
                        return true;
                    }
                }
                return false;
        ...
        function unregisterEventHandler(event, handler, context, selector) {
            ...
            _events = _events.filter(function (eventHandler) {
                for (var i = 0; i < removeHandler.length; i++) {
                    if (eventHandler[i] !== removeHandler[i] && !eqObjects(eventHandler[i], removeHandler[i])) {
                        return true;
                    }
                }
                return false;
    ...
    var WeavyPanels = function (weavy) {
        ...
        function registerLoading(panelId) {
            ...
            if (frame && !frame.registered) {
                ...
                weavy.one(wvy.postal, "ready", { weavyId: weavy.getId(), windowName: frame.name }, onready);
        ...
        function setPanelLoading(panelId, isLoading, fillBackground) {
            if (isLoading) {
                ...
                loadingTimeout[panelId] = weavy.timeout(15000).then(setPanelLoading.bind(weavy, panelId, false));
            } else {
...
(function (root, factory) {
    ...
    var WeavyContext = function (weavy, options, data) {
        ...
        // Event handlers
        context.eventParent = weavy;
        context.on = weavy.events.on.bind(context);
        context.one = weavy.events.one.bind(context);
        context.off = weavy.events.off.bind(context);
        context.triggerEvent = weavy.events.triggerEvent.bind(context);
        ...
        if (appSelector) {
            ...
            if (!app) {
                if (isAppConfig) {
                    ...
                    weavy.warn("App " + (isAppConfig ? JSON.stringify(appSelector) : appOptions) + " does not exist." + (isAppId ? "" : " \n Use weavy.context(" + (context.key && "\"" + context.key + "\"" || context.id || "...") + ").app(" + JSON.stringify(appSelector) + ") to create the app."))
                }
            }
        }
...
(function (root, factory) {
    ...
    var Weavy = function () {
            ...
            if (contextSelector) {
                ...
                if (!context) {
                    if (isContextConfig) {
                        ...
                        weavy.warn("Context " + (isContextConfig ? JSON.stringify(contextSelector) : options) + " does not exist." + (isContextId ? "" : " \n Use weavy.context(" + JSON.stringify(contextSelector) + ") to create the context."))
                    }
                }
            }
        ...
        Object.defineProperty(weavy, "contexts", {
            get: function () {
                return _contexts;
            }
        });
...
Weavy.version = "5.0.1-client+weavy.0896b9af1"; Weavy.defaults.url = "https://hq.weavycloud.com/"; 